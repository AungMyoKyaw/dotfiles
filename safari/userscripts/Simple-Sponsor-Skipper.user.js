...
// @downloadURL https://update.greasyfork.org/scripts/453320/Simple%20Sponsor%20Skipper.user.js
// @updateURL https://update.greasyfork.org/scripts/453320/Simple%20Sponsor%20Skipper.meta.js
// ==/UserScript==
/**
...
(async function () {
  ...
  async function go(videoId) {
    ...
    const favicon = !!document.head.querySelector('link[rel=icon][href]')
      ? document.head.querySelector('link[rel=icon][href]').href
      : undefined; // document.head.querySelector('link[rel=icon][href]')?.href; <-- Syntax error on Pale Moon with Greasemonkey 3
    ...
    const poiNotification = {
      ...
      text:
        'This video has a highlight segment at ' +
        durationString(rPoi) +
    ...
    const vfunc = function () {
      if (
        ...
        location.pathname.indexOf(videoId) === -1 &&
        location.search.indexOf('v=' + videoId) === -1
      ) {
          ...
          if (s3settings.notifications) {
            GM.notification({
              title:
                'Skipped ' +
                result[x].category
                  .replace('music_offtopic', 'non-music')
                  .replace('selfpromo', 'self-promotion') +
                ' segment',
              text:
                'Segment ' +
                (x + 1) +
  ...
  function durationString(scs) {
    ...
    return (
      (durHour > 0 ? durHour + ':' : '') +
      (durHour === 0 || durMin > 9 ? durMin : '0' + durMin) +
      ':' +
      (durSec > 9 ? durSec : '0' + durSec)
    );
  }
  ...
  function processSegments(segments) {
    if (typeof segments === 'object') {
      ...
      if (!!highlight) newSegments.push(highlight);
      return newSegments;
    } else {
      return [];
    }
  ...
  if (!!s3settings && Object.keys(s3settings).length > 0) {
    console.log(
      new Date().toTimeString().split(' ')[0] +
        ' - Simple Sponsor Skipper: Settings loaded!'
    );
    ...
    if (isInt(s3settings.categories)) {
      // converts enum categories to string array
      const cat = [];
      if (s3settings.categories & 2) cat.push('intro');
      if (s3settings.categories & 4) cat.push('outro');
      if (s3settings.categories & 8) cat.push('interaction');
      if (s3settings.categories & 16) cat.push('selfpromo');
      if (s3settings.categories & 32) cat.push('preview');
      if (s3settings.categories & 64) cat.push('music_offtopic');
      if (s3settings.categories & 128) cat.push('filler');
      if (s3settings.categories & 1 || cat.length === 0) cat.push('sponsor');
      if (s3settings.notifications) cat.push('poi_highlight');
    ...
    if (
      navigator.userAgent.toLowerCase().indexOf('pale moon') !== -1 ||
    ...
    console.log(
      new Date().toTimeString().split(' ')[0] +
        ' - Simple Sponsor Skipper: Default settings saved!'
    );
    GM.notification({
        ...
        GM.openInTab(
          document.location.protocol +
            ...
            document.location.pathname
              ...
              .replace('/v/', '/watch?v=') +
            document.location.search.replace('?', '&').replace('&v=', '?v=') +
            '#s3config'
        );
      }
    });
    ...
    window.addEventListener(loadevent, function () {
      ...
      btnSave.addEventListener('click', async function () {
        ...
        if (document.getElementById('sponsor').checked)
          s3settings.categories.push('sponsor');
        if (document.getElementById('intro').checked)
          s3settings.categories.push('intro');
        if (document.getElementById('outro').checked)
          s3settings.categories.push('outro');
        if (document.getElementById('interaction').checked)
          s3settings.categories.push('interaction');
        if (document.getElementById('selfpromo').checked)
          s3settings.categories.push('selfpromo');
        if (document.getElementById('preview').checked)
          s3settings.categories.push('preview');
        if (document.getElementById('music_offtopic').checked)
          s3settings.categories.push('music_offtopic');
        if (document.getElementById('filler').checked)
          s3settings.categories.push('filler');
        else if (s3settings.categories.length === 0)
        ...
        s3settings.upvotes =
          parseInt(document.getElementById('upvotes').value, 10) || -2;
        s3settings.notifications =
          document.getElementById('notifications').checked;
        s3settings.disable_hashing =
          document.getElementById('disable_hashing').checked;
        if (document.getElementById('instance').value.trim() != '')
          s3settings.instance = document
            .getElementById('instance')
            .value.trim();
        s3settings.darkmode = parseInt(
          document.getElementById('darkmode').value,
          10
        );
        await GM.setValue('s3settings', s3settings);
        console.log(
          new Date().toTimeString().split(' ')[0] +
            ' - Simple Sponsor Skipper: Settings saved!'
        );
        ...
        if (location.hostname === 'odysee.com') {
          mutations.forEach(function (mutation) {
            for (let x = 0; x < mutation.addedNodes.length; x++) {
              if (!mutation.addedNodes[x].tagName) continue;
              ...
              if (mutation.addedNodes[x].id === 'vjs_video_3') {
                ...
                if (!!thumb) {
                  ...
                  thumb = thumb
                    .substring(thumb.indexOf('\"') + 1)
                    .split('\"')[0];
                  if (
                    thumb.indexOf('ytimg.com') !== -1 ||
                    thumb.indexOf('img.youtube.com') !== -1
                  ) {
                    go(thumb.split('/vi/').pop().split('/')[0]);
                  } else if (
                    !thumb.toLowerCase().match(/\.(webp|jpeg|jpg|gif|png)$/)
                  ) {
                    go(thumb.split('/').pop());
                  }
                }
          ...
          if (params.has('v') && params.get('v') !== oldVidId) {
            ...
            (location.pathname.indexOf('/embed/') === 0 ||
              location.pathname.indexOf('/v/') === 0) &&
            location.pathname.indexOf(oldVidId) === -1
          ) {
            oldVidId = location.pathname
              .replace('/v/', '')
              .replace('/embed/', '')
