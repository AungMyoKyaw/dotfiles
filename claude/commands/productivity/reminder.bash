...
parse_creation_request() {
    ...
    # Remove common prefixes - be more careful to preserve URLs and context
    reminder_text=$(echo "$reminder_text" | sed -E 's/^(remind me to|add to|add|create|make|new) //i')
    reminder_text=$(echo "$reminder_text" | sed -E 's/^(reminder|task) //i')
    # Don't remove "to" patterns here as they can be part of URLs or meaningful text
    ...
    # List extraction - use the original input to detect lists
    if [[ "$lowercase_input" =~ add\ to\ (work|personal|shopping|home|business|office|learning)\ list ]]; then
        list_name=$(echo "$lowercase_input" | grep -oE "add to (work|personal|shopping|home|business|office|learning) list" | grep -oE "(work|personal|shopping|home|business|office|learning)" | head -1)
    elif [[ "$lowercase_input" =~ (work|personal|shopping|home|business|office|learning)\ list ]]; then
        list_name=$(echo "$lowercase_input" | grep -oE "(work|personal|shopping|home|business|office|learning) list" | grep -oE "(work|personal|shopping|home|business|office|learning)" | head -1)
    fi

    # Clean reminder text by removing extracted elements - more careful with URLs
    ...
    # Use grep to extract URLs more reliably
    local found_urls
    found_urls=$(echo "$temp_text" | grep -oE "$url_pattern" || true)
    ...
    # Remove common patterns but be careful with URLs and context
    clean_text=$(echo "$clean_text" | sed -E 's/\bto read this\b//i')
    ...
    clean_text=$(echo "$clean_text" | sed -E 's/^\s+|\s+$//g')
...
edit_reminder_details() {
    ...
    if [[ -n "$new_list" ]]; then
        DETECTED_PARAMS+=("--list-name" "$new_list")
    fi
